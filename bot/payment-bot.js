...
// Монета для крипто-оплати (за замовчуванням USDT). Ціна тарифу в гривнях
// конвертується в цю монету за курсом CryptoBot.
const CRYPTO_ASSET = (process.env.CRYPTO_ASSET || 'USDT').toUpperCase();
// Запасний курс <монета>→UAH, якщо API курсів недоступне (для USDT).
const FALLBACK_ASSET_UAH = parseFloat(process.env.USDT_UAH_RATE || '');
...
// ============================================================
// КУРС USD→UAH з API Монобанка (для показу гривневого еквіваленту)
// ------------------------------------------------------------
// Використовується ЛИШЕ для відображення «≈ 550₴» у повідомленні.
// Сам рахунок CryptoBot створюється у USD, як і раніше.
// Ліміт Monobank: 1 запит / 60с на IP → кешуємо курс на годину,
// з запасним значенням USD_UAH_FALLBACK (за замовчуванням 42).
// ============================================================
const MONO_CURRENCY_URL = 'https://api.monobank.ua/bank/currency';
const RATE_TTL_MS = 60 * 60 * 1000;       // тримаємо курс годину
const RATE_RETRY_MS = 5 * 60 * 1000;      // після помилки пробуємо за 5 хв
const RATE_FIREBASE_PATH = 'meta/usdUahRate';
const rateCache = { value: Number(process.env.USD_UAH_FALLBACK) || 42, ts: 0, inflight: null };

// Кеш живе лише в пам'яті процесу, а бот під час розробки/деплою рестартує
// частіше, ніж раз на хвилину — після кожного рестарту ts скидався в 0 і бот
// одразу бив по Monobank API знову, хоча курс щойно вже брали. Тому при
// старті процесу підтягуємо останній збережений курс з Firebase — він
// переживає рестарти так само, як лишається валідним годину для будь-якого
// іншого читача.
const rateCacheReady = (async () => {
  try {
    const snap = await get(ref(db, RATE_FIREBASE_PATH));
    const data = snap.val();
    if (data && typeof data.value === 'number' && typeof data.ts === 'number') {
      rateCache.value = data.value;
      rateCache.ts = data.ts;
    }
  } catch (e) {
    // немає збереженого курсу або Firebase недоступний — лишаємось на fallback
  }
})();

async function fetchAndCacheRate() {
  try {
    ...
    const row = Array.isArray(data)
      ? data.find((r) => r.currencyCodeA === 840 && r.currencyCodeB === 980)
      : null;
    const rate = row
      ? (row.rateBuy && row.rateSell ? (row.rateBuy + row.rateSell) / 2 : (row.rateSell || row.rateCross || row.rateBuy))
      : null;
    if (rate) {
      rateCache.value = rate;
      rateCache.ts = Date.now();
      set(ref(db, RATE_FIREBASE_PATH), { value: rate, ts: rateCache.ts }).catch(() => {});
    } else {
      rateCache.ts = Date.now() - (RATE_TTL_MS - RATE_RETRY_MS); // спробуємо ще за 5 хв
    }
  } catch (err) {
    console.error('Курс Monobank недоступний, використовуємо запасний:', err.message);
    rateCache.ts = Date.now() - (RATE_TTL_MS - RATE_RETRY_MS);
  }
  return rateCache.value;
}

async function getUsdUahRate() {
  await rateCacheReady;
  if (rateCache.ts && (Date.now() - rateCache.ts) < RATE_TTL_MS) {
    return rateCache.value;
  }
  // Кілька користувачів можуть одночасно застати прострочений кеш — без
  // цього кожен з них окремо бив би по API в ту саму мілісекунду
  // (thundering herd), і навіть із годинним TTL це й давало «TMR».
  if (!rateCache.inflight) {
    rateCache.inflight = fetchAndCacheRate().finally(() => { rateCache.inflight = null; });
  }
  return rateCache.inflight;
}

// Формує рядок суми для крипто-повідомлень: "12.33 USDT (≈ 550₴)"
async function cryptoAmountLine(plan) {
  const rate = await getUsdUahRate();
  const uah = Math.round(plan.usd * rate);
  return `${plan.usd} USDT (≈ ${uah}₴)`;
}
...
async function sendCryptoInvoice(chatId, plan) {
  try {
    ...
    const amountLine = await cryptoAmountLine(plan);
    ...
    await bot.sendMessage(
      ...
      `🪙 *Оплата криптою (CryptoBot)*\n\n💎 Тариф: ${plan.label}\n💰 Сума: ${amountLine}\n\n1️⃣ Натисніть «Сплатити» і оберіть будь-яку монету (USDT, TON, BTC…)\n2️⃣ Після оплати натисніть «Перевірити оплату»`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: `💳 Сплатити ${plan.usd} USDT`, url: payUrl }],
...
async function sendPaymentMethods(chatId, plan) {
  ...
  const cryptoLine = CRYPTO_ENABLED ? await cryptoAmountLine(plan) : '';

  const text =
    `💎 *${plan.label}*\n\n` +
    '💳 Оберіть спосіб оплати:\n' +
    `⭐ Telegram Stars — ${plan.stars} ⭐\n` +
    (CRYPTO_ENABLED ? `🪙 Крипта (CryptoBot) — ${cryptoLine}\n` : '') +
  ...
  if (CRYPTO_ENABLED) {
    rows.push([{ text: `🪙 Крипта · ${plan.usd} USDT`, callback_data: `crypto:${plan.id}` }]);
  }
...
async function refundCryptoPayment(payment) {
  ...
  try {
    return await cryptoBotCall('transfer', {
    ...
    if (/METHOD_DISABLED/i.test(err.message)) {
      throw new Error('метод «transfer» вимкнено у налаштуваннях застосунку CryptoBot. Увімкніть його: @CryptoBot → Crypto Pay → ваш застосунок → Security/Restrictions. Або поверніть кошти вручну в @CryptoBot.');
    }
    throw err;
  }
}
...
// Екранує спецсимволи звичайного Markdown у динамічному тексті
// (напр. тексті помилки), щоб не ламати розмітку повідомлення.
function escMd(s) {
  return String(s).replace(/([_*`\[])/g, '\\$1');
}

async function editPanel(query, view) {
  const rm = { inline_keyboard: view.keyboard };
  try {
    ...
    // Не вдалося відредагувати (текст не змінився / помилка розмітки) — шлемо нове
    try {
      await bot.sendMessage(query.message.chat.id, view.text, { parse_mode: 'Markdown', reply_markup: rm });
    } catch (e2) {
      // Остання спроба — БЕЗ Markdown, щоб панель точно показалась
      await bot.sendMessage(query.message.chat.id, view.text, { reply_markup: rm }).catch(() => {});
    }
  }
}
...
async function handleAdminCallback(query, data) {
  ...
  if (action === 'rfyes') {
    ...
    try {
      ...
      resultMsg = r.ok ? `✅ ${escMd(r.msg)}` : `⚠️ ${escMd(r.msg)}`;
    } catch (err) {
      console.error('Помилка повернення:', err.message);
      resultMsg = `❌ Не вдалося повернути: ${escMd(err.message)}`;
    }

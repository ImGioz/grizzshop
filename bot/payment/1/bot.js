...
const { getDatabase, ref, set, update, push, get } = require('firebase/database');
...
const REQUIRED_CHANNEL = process.env.REQUIRED_CHANNEL || '@monomoms';
...
🤝 Використовуючи наш сервіс, ви автоматично погоджуєтесь з нашою [Угодою користувача](https://funpapers.net/user_agreement.html) та [Політикою конфіденційності](https://funpapers.net/privacy_policy.html), даєте згоду на обробку ваших даних, підтверджуєте, що вам виповнилось 18 років, та що ви є громадянином України (в цілях безпеки сервісу)

💡 Важливо: Наш сервіс є НЕОФІЦІЙНИМ РЕДАКТОРОМ для створення стилізованих зображень.
...
⚠️ Увага: за нечесні або сейкові відповіді - блокування!`;
...
const PIB_EXAMPLE = 'Іванов Іван';
const PIB_MESSAGE = `🎯 Чудово!

📝 Надішліть, будь ласка:
• ПІБ українською мовою (з великої літери)
...
📝 Введіть ПІБ українською мовою (з великої літери), два слова через пробіл.
...
const PIB_REGEX = /^[А-ЯІЇЄҐ][а-яіїєґ'’-]+\s[А-ЯІЇЄҐ][а-яіїєґ'’-]+$/;
const PIN_REGEX = /^\d{4}$/;
...
async function sendProfile(chatId) {
  ...
  const regDate = user.registrationDate
    ...
    : '—';
  const subLine = user.subscriptionStatus && user.subscriptionEndDate
    ? `Придбана до ${new Date(user.subscriptionEndDate).toLocaleDateString('uk-UA', { timeZone: KYIV_TZ })}`
    : 'Не придбана';
...
// Другий бот отримає /start <planId> і виставить рахунок у Telegram Stars.
function sendSubscriptionPlans(chatId) {
  const text =
    '💎 *Преміум підписка*\n\n' +
    '✨ Що ви отримаєте:\n' +
    '🎯 Ідеальний настрій\n\n' +
...
case 'enter_card':
  if (!/^\d{6}\*{4}\d{4}$/.test(text)) {
    return bot.sendMessage(chatId, '❌ Невірний формат. Використовуйте, наприклад: 414960****2519');
  }
